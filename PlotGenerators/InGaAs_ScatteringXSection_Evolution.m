...
deltaC11oC11 = 1; %deltaC11oC11 = linspace(-0.99, 3, 21);
deltarhoorho = linspace(-0.99, 3, 5);
...
%        [amin(i,j),kmin(i,j),exitflag(i,j)]=fminsearch(@(X) kappa_objective_function(X,T,MatParams_copy),3e-9,options)
        ...
        kvect(j,:) = logspace(log10(kmax)-3,log10(kmax),201)
        [sigma(j,:),scat_eff(j,:)] = GetSigmaSphere(kvect(j,:),1,MatParams_copy)
        figure(1)
        loglog(kvect(j,:)*MatParams_copy.a_NP,scat_eff(j,:),'LineWidth',3)
        hold on
        
        kmax_T = MatParams_copy.kmax(2);
        kvect_T(j,:) = logspace(log10(kmax_T)-3,log10(kmax_T),201)
        [sigma_T(j,:),scat_eff_T(j,:)] = GetSigmaSphere(kvect_T(j,:),2,MatParams_copy)
        figure(2)
        loglog(kvect_T(j,:)*MatParams_copy.a_NP,scat_eff_T(j,:),'LineWidth',3)
        hold on
...
legend('-1','0','1','2','3','Location','SouthEast')
