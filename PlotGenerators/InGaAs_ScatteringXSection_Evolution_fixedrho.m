...
deltaC11oC11 = [-0.5,0.0,0.5,1,2,3];%linspace(-0.98, 3, 10);
deltarhoorho = 0.5;
...
%        [amin(i,j),kmin(i,j),exitflag(i,j)]=fminsearch(@(X) kappa_objective_function(X,T,MatParams_copy),3e-9,options)
        ...
        kvect(i,:) = logspace(log10(kmax)-2,log10(kmax),201)
        [sigma(i,:),scat_eff(i,:)] = GetSigmaSphere(kvect(i,:),1,MatParams_copy)
        figure(1)
        loglog(kvect(i,:)*MatParams_copy.a_NP,scat_eff(i,:),'LineWidth',3)
        hold on
        
        kmax_T = MatParams_copy.kmax(2);
        kvect_T(i,:) = logspace(log10(kmax_T)-2,log10(kmax_T),201)
        [sigma_T(i,:),scat_eff_T(i,:)] = GetSigmaSphere(kvect_T(i,:),2,MatParams_copy)
        figure(2)
        loglog(kvect_T(i,:)*MatParams_copy.a_NP,scat_eff_T(i,:),'LineWidth',3)
        hold on
